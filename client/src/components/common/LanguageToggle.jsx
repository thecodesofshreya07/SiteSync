...
export default function LanguageToggle({ variant = 'light', className = '' }) {
  ...
  return (
    <button
      ...
      className={`relative inline-flex items-center gap-1 sm:gap-1.5 rounded-lg sm:rounded-xl px-1.5 py-0.5 sm:px-2.5 sm:py-1.5 text-[10px] sm:text-xs font-semibold transition-all duration-200 cursor-pointer shadow-2xs select-none shrink-0 ${
        isDark
    ...
    >
      <Languages className={`w-3 h-3 sm:w-4 sm:h-4 shrink-0 ${isHindi ? 'text-[#146b3a]' : 'text-slate-500'}`} />

      {/* Sliding Pill Indicator */}
      <div
        className={`flex items-center gap-0.5 sm:gap-1 rounded-md sm:rounded-lg p-0.5 text-[9px] sm:text-2xs font-bold transition-all ${
          isDark ? 'bg-slate-800/80 text-slate-300' : 'bg-slate-100/90 text-slate-600'
        }`}
      >
        <span
          className={`rounded px-1 py-0.25 sm:px-1.5 sm:py-0.5 transition-all ${
            !isHindi
              ? isDark
